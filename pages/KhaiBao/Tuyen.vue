...
import { routeList } from "~/composables/route/useRouteGlobal";
...
<template>
  <section>
    ...
    <el-table v-loading="loadingData" element-loading-text="Đang tải dữ liệu..." :data="filterTableData"
      ...
      <el-table-column label="Giá cơ bản" prop="base_price" :formatter="(_, __, value) => formatCurrency(value)" />
      ...
      <el-table-column label="Vị trí" width="120">
        <template #default="scope">
          <el-button-group>
            <el-button :disabled="scope.$index === 0" size="small" @click="handleMoveUp(scope.row, scope.$index)"
              :icon="ArrowUp" @click.stop.prevent="" />
    ...
    <el-drawer v-model="drawer" :direction="direction" :before-close="cancelClick">
      ...
      <template #footer>
        <div style="flex: auto">
          ...
          <el-button type="primary" :icon="Checked" :loading="loadingSubmit" @click="handleSubmitRoute(ruleFormRef)">
